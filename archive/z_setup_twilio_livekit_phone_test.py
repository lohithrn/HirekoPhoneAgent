...
def install_cli_tools():
    print("Installing twilio-cli")
    subprocess.run(['npm', 'install', '-g', 'twilio-cli'])

    if "darwin" in os.uname().sysname.lower():
        print("Installing livekit-cli on macOS")
        subprocess.run(['brew', 'update'])
        subprocess.run(['brew', 'install', 'livekit-cli'])
    else:
        print("Skipping livekit-cli installation as the OS is not macOS.")
